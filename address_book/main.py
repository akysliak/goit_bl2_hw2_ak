...
from addressbook import AddressBook, ABIterator
from record import Record
from myexception import MyException, MyIteratorNException
from change import ChangeType, Change
from printer import ExceptionPrinter, SimpleMessagePrinter, WarningPrinter, InputRequest
...
def input_error(fnc):
    def inner(*args):
        try:
            ...
            ExceptionPrinter.print_to_cl(e)
            # print(str(e).replace('"', ""))
            inner()
...
def main_internal():
    while True:
        u_input = InputRequest.print_to_cl() #input(f"{PROMPT} ")
        with warnings.catch_warnings(record=True) as warning_list:
            func, data = command_parser(u_input)
            while not func:
                SimpleMessagePrinter.print_to_cl(
                    "The command is not defined. Please, use a valid command (call 'help' if needed).")
                u_input = InputRequest.print_to_cl() #input(f"{PROMPT} ")
                func, data = command_parser(u_input)
            result = func(data)
        for w in warning_list:
            WarningPrinter.print_to_cl(w) #print(f"\t{WARNING_COLOR}{w.message}{RESET_COLOR}")
        if isinstance(result, ABIterator):
            for el in result:
                SimpleMessagePrinter.print_to_cl(el)
                u_input = InputRequest.print_to_cl("Show more? ([y/n]):") #input("Show more? ([y/n]): ")
                while not (u_input.startswith("y") or u_input.startswith("n")):
                    u_input = InputRequest.print_to_cl("Please, enter one of the options: 'y' to show more results, 'n' to finish this command.\n")
                    #input("Please, enter one of the options: 'y' to show more results, 'n' to finish this command.\n")
                if u_input.startswith("n"):
                    break
            if u_input.startswith("y"):
                SimpleMessagePrinter.print_to_cl("No more results found.")
        else:
            SimpleMessagePrinter.print_to_cl(result)
        if func == exit_handler:
            break


def main(*args):
    SimpleMessagePrinter.print_to_cl("Hello and welcome to the ADDRESS BOOK! How can I help you?\n"
                                     "(Note: you can enter 'help' to get the list of possible commands)")
    new_profile_handler()
    main_internal()
    SimpleMessagePrinter.print_to_cl("Exiting the programm.")  # ("Forwarding to the main menu...")
